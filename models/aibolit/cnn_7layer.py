...
class CNN(nn.Module):
    def __init__(self, config):
        ...
        #layer 5
        conv = nn.Conv1d(shape.channels, filters_num, 6)
        ...
        shape.pool(2, 2)
        self.conv_layers.append(nn.MaxPool1d(2, 2)) #output 18

        #layer 7
        filters_num *= 2
        conv = nn.Conv1d(shape.channels, filters_num, 3)
        shape.conv(filters_num, 3)
        self.conv_layers.append(conv)
        self.conv_layers.append(nn.BatchNorm1d(filters_num))
        self.conv_layers.append(activation()) #output 16

        pad = shape.suggest_padding(8, 8)
        if pad:
            self.conv_layers.append(nn.ConstantPad1d(pad, 0))
        shape.pool(8, 8, padding=pad)
        self.conv_layers.append(nn.AvgPool1d(8, 8))
