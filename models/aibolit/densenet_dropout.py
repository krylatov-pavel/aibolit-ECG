...
class CNN(nn.Module):
    def __init__(self, config):
        ...
        padding = shape.suggest_padding(kernel_size, stride=2, output_length=shape.length // 2)
        if padding:
            self.layers.append(nn.ConstantPad1d(padding, 0))    
        self.layers.append(nn.Conv1d(1, filters, kernel_size, stride=2, bias=False))
        self.layers.append(nn.BatchNorm1d(filters))
        self.layers.append(nn.ReLU())
        shape.conv(filters, kernel_size, stride=2, padding=padding)
        ...
        padding = shape.suggest_padding(pool_size, pool_size)
        if padding:
            self.layers.append(nn.ConstantPad1d(padding, 0))
        self.layers.append(nn.AvgPool1d(pool_size, pool_size))
        shape.pool(pool_size, pool_size, padding)
